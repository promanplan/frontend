...
// Mock site config - replace with actual import when available
...
// Custom hook to handle route changes - replace with actual implementation when available
function useRouteChanged(callback: () => void) {
...
// Simple Logo component using ChakraLogo
function Logo() {
  return <ChakraLogo height="32px" />
}
            ...
            <Box>
              ...
              <Stack alignItems="stretch" spacing="0">
                {siteConfig.header.links.map(
                  ({ href, id, label, ...props }: SiteConfigLink, i: number) => {
                    return (
                      <NavLink
                        href={href || `/#${id}`}
                        key={i}
